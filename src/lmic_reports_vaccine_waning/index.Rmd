...
if(identical(params$adjust_delta, FALSE)){
  ...
  delta_text <- paste0(
    "This report adjusts for the Delta variant, with the variant become dominant over the period: ",
    params$adjust_delta$delta_start_date,
    " to ",
    params$adjust_delta$delta_start_date + params$adjust_delta$delta_shift_duration, 
    "."
  )
  if(!params$adjust_delta$omicron_imputed){
    delta_text <- paste0(delta_text,
    " Additionally this report adjusts for the oOmicron variant, with the variant become dominant over the period: ",
    params$adjust_delta$omicron_start_date,
    " to ",
    params$adjust_delta$omicron_start_date + params$adjust_delta$omicron_shift_duration, 
    "."
  )
  }
}
...
Whilst the scenarios are moved over to the new method, the previous 50% increase/decrease
scenarios will be presented here.

```{r healthcare plots legacy, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 7: Healthcare demands in the next 28 days.** Individuals needing an ICU bed are assumed to need mechanical ventilation. Projected demand for Scenario 1 (the epidemic continues to grow at the current rate) are shown in green (Maintain status quo). Projections for Scenario 2 (a further 50% reduction in transmission) are shown in blue. Projections for Scenario 3 (relaxing interventions by 50%) are shown in red. Current date shown with dashed line."), fig.width=fig_height(8,10), fig.height=fig_height(3.5,4.5), fig.align=fig_center()}
# suppressWarnings(suppressMessages(cowplot::plot_grid(
#   healthcare_plot_contrast(o1 = params$o_list[[1]], o2 = params$o_list[[2]], 
#                            data = data, date_0 = date_0, date = date, 
#                            forecast = 28, what = "hospital_demand"),
