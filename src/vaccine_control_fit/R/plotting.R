...
fitting_plot <- function(res, country, date_0){
  ...
  # get ECDC hospital data
  owid <- read.csv("https://covid.ourworldindata.org/data/owid-covid-data.csv")
  hosp_dat <- owid %>% select(date, hosp_patients, iso_code, location) %>% 
    rename(country = location,
           iso3c = iso_code,
           value = hosp_patients) %>% 
    mutate(indicator = "Daily hospital occupancy")
  hosp_dat$date <- as.Date(hosp_dat$date)
  
  # Hosp default
  hosp_demand_plot <- function(out, hosp_dat, cc, end_date = NULL) {
    
    if(is.null(end_date)) {
      end_date <- max(out$pmcmc_results$inputs$data$date)
    }
    
    # Hopsital Data
    cc_icu <- (hosp_dat %>% filter(country == cc & indicator == "Daily hospital occupancy"))
    
    # ICU demand
    sub <- nim_sq_format(out, c("hospital_occupancy","ICU_occupancy"), date_0 = max(out$pmcmc_results$inputs$data$date)) %>% 
      na.omit() %>% 
      group_by(date, t, replicate) %>% 
      summarise(y = sum(y, na.rm=TRUE)) %>% 
      group_by(date) %>% summarise(
        ymin = quantile(y, 0.025, na.rm=TRUE),
        yinner_min = quantile(y, 0.25, na.rm=TRUE),
        yinner_max = quantile(y, 0.75, na.rm=TRUE),
        ymax = quantile(y, 0.975, na.rm=TRUE),
        ymed = median(y, na.rm = TRUE),
      ) %>% 
      mutate(compartment = "Hospital Demand") %>% 
      filter(date <= end_date)
    
    # Plot
    gg <- ggplot2::ggplot(sub, ggplot2::aes(x = .data$date,
                                            col = .data$compartment)) +
      geom_bar(aes(x = as.Date(date), y = value, fill = "Reported"),
               data = cc_icu %>% filter(date <= end_date),
               stat = "identity",
               inherit.aes = FALSE, 
               show.legend = FALSE) +
      ggplot2::geom_ribbon(mapping = ggplot2::aes(ymin = .data$ymin,
                                                  ymax = .data$ymax,
                                                  fill = "Estimated"),
                           color = "white",
                           alpha = 0.2,
                           size = 0,
                           show.legend = TRUE) +
      ggplot2::geom_ribbon(mapping = ggplot2::aes(ymin = .data$yinner_min,
                                                  ymax = .data$yinner_max,
                                                  fill = "Estimated"),
                           color = "white",
                           alpha = 0.8,
                           size = 0,
                           show.legend = TRUE) +
      ggplot2::geom_line(mapping = ggplot2::aes(y = .data$ymed,
                                                fill = "Estimated"),
                         color = "black",
                         alpha = 0.8,
                         show.legend = TRUE) +
      
      ggplot2::ylab("Hospital Demand") +
      ggplot2::theme_bw()  +
      ggplot2::scale_y_continuous(expand = c(0,0)) +
      ggplot2::scale_fill_manual(name = "", labels = (c("Estimated", "Reported")),
                                 values = (c("#3f8ea7","#c59e96"))) +
      ...
      ) + ggtitle(out$parameters$country)
    
    return(gg) 
  }
  
  # Deaths default
  death_plot <- function(out, end_date = NULL) {
    ...
    # Deaths
    sub <- nim_sq_format(out, "deaths", date_0 = max(out$pmcmc_results$inputs$data$date)) %>% 
      na.omit() %>% 
      group_by(date) %>% summarise(
        ymin = quantile(y, 0.025, na.rm=TRUE),
        yinner_min = quantile(y, 0.25, na.rm=TRUE),
      ...
      mutate(compartment = "deaths") %>% 
      filter(date <= end_date)
    
    # Plot
    gg <- ggplot2::ggplot(sub, ggplot2::aes(x = .data$date,
                                            col = .data$compartment)) +
      geom_bar(aes(x = as.Date(date), y = deaths, fill = "Reported"),
  ...
  # Combined fit
  demand_plot <- function(out, hosp_dat, cc, end_date = NULL) {
    
    hosp_plot <- hosp_demand_plot(out, hosp_dat, cc, end_date)
    death_plot <- death_plot(out, end_date)
    
    comb <- cowplot::plot_grid(death_plot,
                               hosp_plot + theme(plot.title = element_blank()), 
                               ncol = 1) 
  ...
  demand_plot(res, hosp_dat, country, end_date = date_0)
