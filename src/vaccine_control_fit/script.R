...
vacc_inputs <- readRDS("vacc_inputs.rds")
vacc_inputs <- vacc_inputs[[vaccine_scenario]]
keep_vacc <- which(vacc_inputs$date_vaccine_change <= date_0)
vacc_inputs$date_vaccine_change <- vacc_inputs$date_vaccine_change[keep_vacc]
vacc_inputs$max_vaccine <- vacc_inputs$max_vaccine[keep_vacc]
vacc_inputs$vaccine_efficacy_infection <- vacc_inputs$vaccine_efficacy_infection[keep_vacc]
vacc_inputs$vaccine_efficacy_disease <- vacc_inputs$vaccine_efficacy_disease[keep_vacc]

# Defaults for now. 
strategy <- "HCW, Elderly and High-Risk"
available_doses_proportion <- as.numeric(available_doses_proportion)

vaccine_uptake <- as.numeric(vaccine_uptake)
...
out <- squire::pmcmc(data = data, 
                     ...
                     dur_R = as.numeric(dur_R),
                     dur_V = as.numeric(dur_V))
...
# make a fitting plot
fitting_plot <- fitting_plot(out, country, date_0)
ggsave("fitting.pdf", fitting_plot, width = 14, height = 10)
