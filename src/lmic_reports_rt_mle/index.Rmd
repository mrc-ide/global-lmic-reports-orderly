...
infections <- params$o_list$r_central_scenario %>% 
...
hospital <- params$o_list$r_central_scenario %>% 
...
icu <- params$o_list$r_central_scenario %>% 
...
deaths <- params$o_list$r_central_scenario %>% 
...
infections_mit <- params$o_list$r_optimistic_scenario %>% 
  filter(compartment == "infections")
infections_mit_28 <- infections_mit %>% 
  filter(date %in% past_dates)
...
infs_today_mit <- group_by(infections_mit[infections_mit$date==date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

infs_28_mit <- group_by(infections_mit[infections_mit$date == projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

hospital_mit <- params$o_list$r_optimistic_scenario %>% 
  filter(compartment == "hospital_demand")
hosp_today_mit <- group_by(hospital_mit[hospital_mit$date == date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

hosp_28_mit <- group_by(hospital_mit[hospital_mit$date == projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

icu_mit <- params$o_list$r_optimistic_scenario %>% 
  filter(compartment == "ICU_demand")
icu_today_mit <- group_by(icu_mit[icu_mit$date == date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

icu_28_mit <- group_by(icu_mit[icu_mit$date == projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

deaths_mit <- params$o_list$r_optimistic_scenario %>% 
  filter(compartment == "deaths")
deaths_28_mit <- group_by(deaths_mit[deaths_mit$date == projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])
...
infections_rev <- params$o_list$r_pessimistic_scenario %>% 
  filter(compartment == "infections")
infections_rev_28 <- infections_rev %>% 
  filter(date %in% past_dates)
...
infs_28_rev <- group_by(infections_rev[infections_rev$date==projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

infs_today_rev <- group_by(infections_rev[infections_rev$date==date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])
...
hospital_rev <- params$o_list$r_pessimistic_scenario %>% 
  filter(compartment == "hospital_demand")
hosp_today_rev <- group_by(hospital_rev[hospital_rev$date==date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

hosp_28_rev <- group_by(hospital_rev[hospital_rev$date==projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

icu_rev <- params$o_list$r_pessimistic_scenario %>% 
  filter(compartment == "ICU_demand")
icu_today_rev <- group_by(icu_rev[icu_rev$date==date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

icu_28_rev <- group_by(icu_rev[icu_rev$date==projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])

deaths_rev <- params$o_list$r_pessimistic_scenario %>% 
  filter(compartment == "deaths")
deaths_28_rev <- group_by(deaths_rev[deaths_rev$date==projection_end_date,], replicate) %>% 
  summarise(tot = sum(y, na.rm = TRUE)) %>% 
  summarise(i_tot = mean(tot, na.rm = TRUE), 
            i_min = t_test_safe(tot)$conf.int[1],
            i_max = t_test_safe(tot)$conf.int[2])
...
```{r set up delta, echo = FALSE, collapse = TRUE, warning = FALSE}
if(identical(params$adjust_delta, FALSE)){
  delta_text <- ""
} else {
  delta_text <- paste0(
    "This report adjusts for the Omicron and Delta variants, with the variants becoming dominant over the period: ",
    params$adjust_delta$Delta$start_date,
    " to ",
    params$adjust_delta$Delta$start_date + params$adjust_delta$Delta$shift_duration, 
    " for Delta, and ",
    params$adjust_delta$Omicron$start_date,
    " to ",
    params$adjust_delta$Omicron$start_date + params$adjust_delta$Omicron$shift_duration, 
    " for Omicron. These timings are informed by sequence data taken from [CoVariants](https://covariants.org/)."
  )
}
```
...
suppressWarnings(suppressMessages(print(cases_plot(params$o_list[[1]], data = df_cases, date = date, date_0 = date_0))))
...
rts <- map(split(rt_futures, rt_futures$scenario), 
    ~paste0(signif(.x$med, 3), " (", 
                      signif(.x$q_025, 3), ", ",
                      signif(.x$q_975, 3), ")")
)
```
...
We include short-term projections of healthcare demand and new infections under the following three scenarios:

* **Scenario 1, Central:** $R_{t}$ will continue its median trend, reaching `r rts$central` by the end of the 28 day projection period. 
* **Scenario 2, Optimistic:** The $R_{t}$ value will continue its $25\%$ quantile trend, reaching `r rts$optimistic`.
Representing either the implementation of new interventions, scaling up of interventions, or a reduction in mixing.
* **Scenario 3, Pessimistic:** The $R_{t}$ value will continue its $75\%$ quantile trend, reaching `r rts$pessimistic`.
Representing either the relaxation of interventions, or an increase in mixing.

These trends were calculated by looking at all 28 day changes in the fitted $R_t$ values and then calculating the relevant quantiles within each replicate.
...
**N.B. These scenarios are unlikely to show significant differences for the first week since there is a delay of approximately 10 days between infection and hospital admission. Consequently, the effectiveness of a change in policy is likely to be better captured by hospital admission data approximately 2 weeks after the policy change is implemented.**

<br>

```{r healthcare plots, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 5: Healthcare demands in the next 28 days.** Individuals needing an ICU bed are assumed to need mechanical ventilation. Projected demand for Scenario 1 (Central) are shown in green . Projections for Scenario 2 (Optimistic) are shown in blue. Projections for Scenario 3 (Pessimistic) are shown in red. Current date shown with dashed line."), fig.width=fig_height(8,10), fig.height=fig_height(3.5,4.5), fig.align=fig_center()}

combined_projections <- c("r_central_scenario", "r_optimistic_scenario", "r_pessimistic_scenario")
names(combined_projections) <- combined_projections
combined_projections <- map_dfr(combined_projections,
                                ~params$o_list[[.x]], .id = "Scenario")

suppressWarnings(suppressMessages(gg_hosp <- healthcare_plot_contrast(
  projs = combined_projections, what = "hospital_demand", date = date, forcast = 28
)))
suppressWarnings(suppressMessages(gg_icu <- healthcare_plot_contrast(
  projs = combined_projections, what = "ICU_demand", date = date, forcast = 28
)))
...
```{r infections forecast plots, echo = FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.cap=figRef("**Figure 6: Daily number of infections estimated by fitting to deaths.** Projected infections for Scenario 1 (Central) are shown in green. Projections for Scenario 2 (Optimistic) are shown in blue. Projections for Scenario 3 (Pessimistic) are shown in red. Current date shown with dashed line."), fig.width=7, fig.height=fig_height(4,4.5), fig.align=fig_center()}


suppressWarnings(suppressMessages(healthcare_plot_contrast(
  projs = combined_projections, what = "infections", date = date, forcast = 28
)))

```

### Legacy Short Term Scenarios

Legacy scenarios with flat $50\%$ increase/decrease to $R_t$ are presented here.

```{r healthcare plots legacy, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 7: Healthcare demands in the next 28 days.** Individuals needing an ICU bed are assumed to need mechanical ventilation. Projected demand for Maintain (the epidemic continues to grow at the current rate) are shown in green. Projections for Migitation (a further 50% reduction in transmission) are shown in blue. Projections for Reverse (relaxing interventions by 50%) are shown in red. Current date shown with dashed line."), fig.width=fig_height(8,10), fig.height=fig_height(3.5,4.5), fig.align=fig_center()}

combined_projections_leg <- c("r_maintain_scenario_leg", "r_mitigation_scenario_leg", "r_reverse_scenario_leg")
names(combined_projections_leg) <- combined_projections_leg
combined_projections_leg <- map_dfr(combined_projections_leg,
                                ~params$o_list[[.x]], .id = "Scenario")

suppressWarnings(suppressMessages(gg_hosp_leg <- healthcare_plot_contrast(
  projs = combined_projections_leg, what = "hospital_demand", date = date, forcast = 28
)))
suppressWarnings(suppressMessages(gg_icu_leg <- healthcare_plot_contrast(
  projs = combined_projections_leg, what = "ICU_demand", date = date, forcast = 28
)))
leg <- suppressWarnings(suppressMessages(
  cowplot::get_legend(gg_hosp_leg + theme(legend.justification = c(0.5,1)))
))
bottom <- suppressWarnings(suppressMessages(
  cowplot::plot_grid(gg_hosp_leg + theme(legend.position = "none"), 
                     gg_icu_leg + theme(legend.position = "none"), ncol = 2)
))
suppressWarnings(suppressMessages(grid <- cowplot::plot_grid(leg,bottom,nrow=2,rel_heights = c(0.1,1))))
suppressWarnings(suppressMessages(print(grid)))
```

```{r infections forecast plots legacy, echo = FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.cap=figRef("**Figure 8: Daily number of infections estimated by fitting to deaths.** Projected infections for Maintain (the epidemic continues to grow at the current rate) are shown in green. Projections for Mitigation (a further 50% reduction in transmission) are shown in blue. Projections for Reverse (relaxing interventions by 50%) are shown in red. Current date shown with dashed line."), fig.width=7, fig.height=fig_height(4,4.5), fig.align=fig_center()}
suppressWarnings(suppressMessages(healthcare_plot_contrast(
  projs = combined_projections_leg, what = "infections", date = date, forcast = 28
)))
```
