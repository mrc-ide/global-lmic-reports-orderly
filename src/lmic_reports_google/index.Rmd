...
```{r fig1, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 1: Cumulative Deaths since 10 deaths.** Country not shown if fewer than 10 deaths."), fig.height=fig_height(4.5,6), fig.width=fig_height(6,8), fig.align=fig_center()}
...
Using the model fit, we can forecast the expected trajectory for cumulative deaths, which is shown in the figure below. This assumes a severity pattern by age that is consistent with that observed in China, Europe and the U.S to date. This projection assumes that transmission is maintained at the current level of transmission as estimated by the final $R_t$ value. `r show_capacity_sentence`
