...
```{r fig1, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 1: Cumulative Deaths since 10 deaths.** Note if 10 deaths are yet to be reported, country will not be shown."), fig.height=fig_height(3.24,6), fig.width=fig_height(6,8), fig.align=fig_center()}
...
```{r death forecast plots, echo = FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.cap=figRef("**Figure 3: Daily Deaths estimated by fitting to the current total of deaths.** Projected deaths assuming the current level of interventions are maintained are shown in red (mean and 95% quantile)"), fig.width=8, fig.height=fig_height(4,4.5), fig.align=fig_center()}
...
We make the following short-term projections of healthcare demand and new cases under the the following three scenarios:
...
We estimate that over the next 2 weeks demand for hospital beds will change from `r form(hosp_today)` patients requiring treatment with high-pressure oxygen at the current date to `r form(hosp_14)` hospital beds being required on `r format(date + 14, "%Y-%m-%d")` if no further interventions are introduced (Scenario 1). Similarly, we estimate that over the next 2 weeks demand for critical care (ICU) beds will change from `r form(icu_today)` patients requiring treatment with mechanical ventilation at the current date to `r form(icu_14)` by `r format(date + 14, "%Y-%m-%d")`. These projections assume that approximately 5% of all infections will require treatment with high-pressure oxygen and that approximately 30% of hospitalised cases will require require treatment with mechanical ventilation (based on analysis of ongoing epidemics in Europe). **N.B. The difference between each scenario is unlikely to be significant due to mean duration from infection to hospital admission assumed to be approximately 10 days. Consequently, we advise that the effectiveness of a change in policy by measuring healthcare burden should be made after 14 days.**

<br>

```{r healthcare plots, echo = FALSE, collapse=TRUE, message = FALSE, warning = FALSE, fig.cap=figRef("**Figure 4: Healthcare demands in the next 2 weeks.** Individuals needing an ICU bed are predicted to need mechanical ventilation. Projected demand for Scenario 1 (the epidemic continues to grow at the current rate) are shown in green (Maintain Staus Quo). Projections for Scenario 2 (a further 50% reduction in transmission) are shown in blue. Projections for Scenario 3 (relaxing interventions by 50%) are shown in red. The dashed line shows the current day."), fig.width=fig_height(8,10), fig.height=fig_height(4.5,5.5), fig.align=fig_center()}
...
The impact of each scenario has a more immediate effect on the daily number of cases. The figure below shows the impact of each scenario on the estimated daily incidence of new cases. If interventions are scaled up (Scenario 2), the daily number of cases will change from `r form(infs_today)` at the current date to `r form(infs_14_mit)` by `r format(date + 14, "%Y-%m-%d")`. If current interventions were relaxed by 50%, we estimate the daily number of cases will change from `r form(infs_today)` at the current date to `r form(infs_14_rev)` by `r format(date + 14, "%Y-%m-%d")`.

<br>

```{r cases forecast plots, echo = FALSE, collapse=TRUE, warning=FALSE, message=FALSE, fig.cap=figRef("**Figure 5: Daily number of infections estimated by fitting to the current total of deaths.** Projected cases for Scenario 1 (the epidemic continues to grow at the current rate) are shown in green (Maintain Staus Quo). Projections for Scenario 2 (a further 50% reduction in transmission) are shown in blue. Projections for Scenario 3 (relaxing interventions by 50%) are shown in red. The dashed line shows the current day."), fig.width=7, fig.height=fig_height(4,4.5), fig.align=fig_center()}
