...
```{r eval=FALSE, message=FALSE, warning = FALSE}
import <- map(tasks, orderly::orderly_bundle_import)
...
These scripts update the fits in `gh-fits`.
...
```{bash}
./scripts/update_github_fits.sh 2022-05-30
```
...
```{bash}
./run/run_collate_vaccine_waning.sh 2022-05-30
```
