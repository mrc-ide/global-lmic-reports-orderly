...
iso3cs <- setdiff(read_csv("Status_runs_March13.csv")$country, "TOTAL COVERED")
iso3cs <- setdiff(iso3cs, str_remove(list.files("gh-fits"), ".Rds"))
...
```{r eval=FALSE, message=FALSE, warning = FALSE}
iso3cs <- names(bundles)[purrr::map_lgl(bundles, ~file.exists(stringr::str_replace(.x$path, "raw", "derived")))]
