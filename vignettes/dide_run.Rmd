...
#for now we cannot adjust for delta/waning in standard fit
if(!excess_mortality){
  adjust_delta <- FALSE
  waning <- FALSE
}
...
iso3cs_90_uptake <- c("ESP", "THA", "AUS", "COL")
bundles_90_uptake <- lapply(
  iso3cs_90_uptake, 
  function(x) {
    parameters$iso3c <- x
    orderly::orderly_bundle_pack(
...
names(bundles_90_uptake) <- iso3cs_90_uptake
saveRDS(bundles_90_uptake, file.path(path_bundles, "bundles_90_uptake.rds"))

iso3cs_95_uptake <- c("ESP", "THA", "AUS", "COL")
bundles_95_uptake <- lapply(
  iso3cs_95_uptake, 
  function(x) {
    parameters$iso3c <- x
    orderly::orderly_bundle_pack(
...
names(bundles_95_uptake) <- iso3cs_95_uptake
saveRDS(bundles_95_uptake, file.path(path_bundles, "bundles_95_uptake.rds"))
```
...
```{bash}
 ./run/run_collate_vaccine_waning.sh 2021-10-06
```
