...
sequencing data from [NextStrain](https://nextstrain.org/).
...
input_sequencing_id <- orderly::orderly_run("input_sequencing", parameters = list(date=date), echo = FALSE)
...
qpdf::pdf_combine(
  ...
  output = file.path("fits", paste0("lmic_reports_rt_optimise_", date, ".pdf"))
)
```
...
There is also a more streamlined [repo](https://github.com/mrc-ide/global_lmic_projections_esft)
to collect the projections for use in the departments version of the ESFT, which
we will clone into `gh-esft`.
...
#requires git bash be on the path and that the working directory is set correctly
log <- shell(here::here("scripts/update_github_pages.sh"), intern = TRUE, wait = TRUE, shell = "bash")
```

## Update projections in `gh-esft`

We also need to update the streamlined version of the outputs.

```{r eval=FALSE, message=FALSE, warning = FALSE}
#put projections into R format and trim
update_gh_esft(here::here("gh-pages"), here::here("gh-esft"))

#requires git bash be on the path and that the working directory is set correctly
shell(paste0(here::here("scripts/update_github_esft.sh"), " ", date), intern = TRUE, wait = TRUE, shell = "bash")
```
