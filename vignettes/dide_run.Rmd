...
glodide_loc <- "Z:/lmic_fitting/glodide" ###Update if on a different drive!
```
...
iso3cs <- c("CAN","DZA","HRV","MYS","LKA","TTO","ABW","BTN","BWA")

#iso3cs <- sample (iso3cs, 25) #Testing batch
...
saveRDS(bundles, file.path(path_bundles, "bundles_rerun.rds"))
```
...
paths <-list.files(path = paste0("Z:/lmic_fitting/glodide/analysis/data/derived/",date), pattern = ".zip")

# now extract the fitting.pdf files
td <- tempdir(check = TRUE)
fits <- lapply(paths, function(x) {
  
  y <- paste0("Z:/lmic_fitting/glodide/analysis/data/derived/",date,"/",x)
  if(file.exists(paste0(y))){
    tryCatch(zip::unzip(
      zipfile = y, 
      files = file.path(gsub("\\.zip", "", basename(x)), c("pack/excess_fitting.pdf", "pack/reported_fitting.pdf")), 
      exdir = td), error=function(e) NULL)
...
bundles_real <- readRDS(file.path(path_bundles, "bundles.rds"))

# If bundles file is lost: 

paths <-list.files(path = paste0("Z:/lmic_fitting/glodide/analysis/data/derived/",date), pattern = ".zip") # Folder where all the .zip outputs file are located 
bundles <- vector(mode = "list", length = length(paths))
names <- vector(length=length(paths))

for ( i in 1:length(bundles)){
  
  path <- paths[[i]]
  
  y <- paste0("Z:/lmic_fitting/glodide/analysis/data/derived/",date,"/",path)
  id <- gsub("\\.zip", "", basename(path))
  
  unzip_file <- zip::unzip(
    zipfile = y, 
    files = file.path(id, "pack/excess_out.Rds"), exdir=td)
  
  obj <- read_rds(paste0(td,"/",id,"/pack/excess_out.Rds"))
  
  bundles[[i]]$id <- id
  bundles[[i]]$path <- y
  names[i] <- obj$inputs$data$iso[1]
  
}
names(bundles) <- names #Naming each bundle with their ISO 
...
report_ids <- gather_report_ids(task, iso3cs)
update_gh_fits(report_ids)
